...
export default function Bundle() {
    ...
    // Fetches the books from the database
    useEffect(() => {
    ...
    function handleAddCart() {
        books.filter(book => book.tom === link).forEach(book => {
            addToCart(book)
        })
    }
    ...
    const produtosFiltrados = books.filter(product => product.tom === link)
                ...
                return (
                    <div key={item.tom} className='flex flex-col py-8 px-8 lg:px-64 lg:py-32 gap-8'>
                        ...
                        <div className='border-gray-100 bg-white rounded-2xl flex flex-col lg:flex-row justify-center gap-8 lg:p-8 shadow-sm py-8'>
                            ...
                            {/* INFO */}
                            <div className='flex flex-col lg:w-1/2 gap-4 px-4 lg:px-0'>
                                <div>
                                ...
                                <button onClick={handleAddCart} className={` ${item.bgColor}  rounded-full p-2 text-lg text-white font-semibold flex gap-2 justify-center items-center hover:${item.bgColor}/30  transition-colors`}>
                    ...
                    {books.filter(book => book.tom === link).map((book) => {
                        return (
                            <div key={book.id} className='w-full gap-2 flex flex-col p-4 bg-gray-50 rounded-xl border border-gray-100'>
                                <h3 className='text-lg font-semibold border-b pb-2 text-gray-800'>{book.nome}</h3>
                                <div className='text-sm flex flex-col gap-1 text-gray-600'>
                                    <div><span className='font-semibold text-gray-800'>Editora:</span> {book?.livros?.editora || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>ano:</span> {book?.ano || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>Idioma:</span> {book?.livros?.idioma || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>Páginas:</span> {book?.livros?.quantidade_paginas || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>ISBN-10:</span> {book?.livros?.isbn10 || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>ISBN-13:</span> {book?.livros?.isbn13 || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>Peso:</span> {book?.livros?.peso || 'N/A'}</div>
                                    <div><span className='font-semibold text-gray-800'>Dimensões:</span> {book?.livros?.dimensoes || 'N/A'}</div>
                                </div>
                            </div>
                        )
                    })}
