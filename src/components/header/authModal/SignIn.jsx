...
export default function SignIn({setIsAuthModalOpen, onSignIn, setIsSignUp}){
    ...
    return (
        <div className='z-50 bottom-14 left-0 absolute w-full h-fit p-5 bg-white border-gray-700 shadow-lg lg:w-70 lg:left-auto lg:right-0 lg:top-14 rounded-2xl gap-3 flex flex-col items-start '>
            <form onSubmit={handleSubmit} className="flex flex-col gap-2 w-full">
                <input value={email} placeholder='E-mail' onChange={handleEmailChange} onBlur={handleBlur} className={`w-full px-2 border ${hasEmailError ? 'border-red-500': 'border-gray-400'} rounded-lg h-8 type="email`}/>
                {showEmailError && <p className='text-xs text-red-500'>Digite o email</p>}
                <input value={password} placeholder='Senha' onChange={(e)=> setPassword(e.target.value)} className={`w-full px-2 border ${hasPasswordError ? 'border-red-500': 'border-gray-400'} rounded-lg h-8 type="email`} type="password" onBlur={handlePasswordBlur}/>
